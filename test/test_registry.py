...
def img_cmp_old(img1_name, img2_name): # TODO REMOVE
...
def img_cmp_new(img1_name, img2_name): # TODO make official version
    with open(img1_name, 'r') as img1_src:
        with open(img2_name, 'r') as img2_src:
            img1 = img1_src.read().split()
            img2 = img2_src.read().split()
            val1 = [int(x) for x in img1[1:]]
            val2 = [int(x) for x in img2[1:]]
            return img1[0] == img2[0] and val1 == val2

def img_cmp(img1_name, img2_name): # TODO REMOVE
    return img_cmp_new(img1_name, img2_name) or img_cmp_old(img1_name, img2_name)
...
def test_carve_eval(tu, tn, args_ref_out): # TODO REMOVE
    args, ref_file, out_file = args_ref_out
    carve_bin = tu.join_base(carve_path)
    if not os.path.exists(carve_bin):
        return tu.FAILURE("carve binary not available")
    ...
    if check_res:
        if not os.access(out_file, os.R_OK):
            return tu.FAILURE('no "out.ppm" produced\n')
        if img_cmp_new(out_file, ref_file):
            return tu.SUCCESS()
        else:
            return tu.FAILURE('incorrect output image')
    else:
...
pre_tests = {
    'public.statistics.small1_s',
    'public.statistics.small2_s',
    'public.statistics.owl_s',
    'public.min_path.small1_m',
    'public.min_path.small2_m',
    'public.min_path.owl_m',
...
assert(len(pre_tests) == len(set(map(lambda x: x.split('.')[-1], pre_tests))))
...
for t in pre_tests:
    ...
    if ex == 'statistics':
        if (case.startswith('imgbroken')):
            ...
            all_tests[t] = specialize(test_literal, (['-s', case_path], ref_prefix + case_file + '.statistics', 'incorrect statistics'))
    elif ex == 'min_path':
        all_tests[t] = specialize(test_literal, (['-p', case_path], ref_prefix + case_file + '.path', 'incorrect minimal path'))
    else:
        assert(ex == 'carve')
        num = cn[1]
        if (cat == "secret"): # TODO remove case distinction
            all_tests[t] = specialize(test_carve_eval, (['-n', num, case_path], 'test/ref_output/' + case + '.ppm', 'out.ppm'))
        else:
