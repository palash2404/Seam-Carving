...
result_t energy_small2_test(const char *test) {
  ...
  for (uint32_t i = 0; i < w * h; i++) {
    if (energy[i] != ref_energy[i]) {
      printf("energy at %u: %u\nref energy: %u", i, energy[i], ref_energy[i]);
      res = FAILURE;
      break;
    }
  }
...
result_t optimal_path_tall_test(const char *test) {
  ...
  for (int i = 5; i >= 0; i--) {
    if (seam[i] != exp_seam[i]) {
      printf("at row %d: expected seam column %u, but got %u", i, exp_seam[i],
             seam[i]);
      res = FAILURE;
      break;
    }
  ...
  for (int x = 0; x < 3; x++) {
    for (int y = 0; y < 3; y++) {
      ...
      if (p.r != exp_p.r || p.g != exp_p.g || p.b != exp_p.b) {
        printf("at row %d, column %d: expected %d %d %d, but got %d %d %d", y,
               x, exp_p.r, exp_p.g, exp_p.b, p.r, p.g, p.b);
        res = FAILURE;
      }
    }
  }
